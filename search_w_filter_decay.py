...
class BacktestSearcher:
    ...
    def search(self, query, cutoff_date=None, stock_list=None, use_decay=True, top_k=10):
        ...
        # --- 4. RRF FUSION WITH DECAY ---
        rrf_scores = {}
        
        def add_score(doc_id, rank):
            # Base Score
            ...
            if use_decay:
                ...
                if doc_data:
                    ...
                    final_score = self.calculate_recency_score(base_score, date_str, cutoff_date)
                else:
                    final_score = base_score
            else:
                final_score = base_score

            rrf_scores[doc_id] = rrf_scores.get(doc_id, 0) + final_score

        for rank, doc_id in enumerate(bm25_results_ids):
            add_score(doc_id, rank)
        for rank, doc_id in enumerate(vector_results_ids):
            add_score(doc_id, rank)

        # Sort Final Results
        final_results = sorted(rrf_scores, key=rrf_scores.get, reverse=True)[:top_k]
        ...
        print(f"\n--- HYBRID RESULTS (Decay={use_decay}) ---")
        for i, doc_id in enumerate(final_results):
            data = content_map.get(doc_id)
            if data:
                text = data['text'][:85].replace("\n", " ") + "..."
                meta = data['metadata']
                print(f"{i+1}. [{meta.get('date')}] {meta.get('stock')}: {text}")
            else:
                print(f"{i+1}. [Missing Data] {doc_id}")
